                ...
                if (mysqli_num_rows(mysqli_query($conn, $query)) > 0) {
                    ...
                    if ($multiplier == 1) {
                        ...
                        if ($rowResources['respeito'] >= 5) {
                            $respeito = $rowResources['respeito']-5;
                            $query = "UPDATE resources SET respeito=$respeito WHERE id_user = $id AND respeito >= 5";
                            if (mysqli_query($conn, $query)) {
                                $query = "UPDATE user_app_upgrade SET multiplier='1', lives='1', date_upgraded='$data' WHERE id_user = $id";
                                if (mysqli_query($conn, $query)) {
                                    $msg = "You upgrade your app";
                                    $erro = false;
                                } else {
                                    $msg = "ERROR APP UPGRADE 1!";
                                }
                            } else {
                                $msg = "ERROR APP UPGRADE 2!";
                            }
                        } else {
